...
class Recipe:
    ...
    def dispaly_recipe(self):
        print(f'Name:{self.name}')
        print('Ingredients:')
        for item, qty in self.ingredients.items():
            print(f' -{item}: {qty}')
        print('Instructions:')
        print(f'{self.instructiions}')
        print(f'Serving:{self.serving}')
...
mapo_tofu = Recipe(
    ...
    instructions="1. Stir-fry pork with doubanjiang. 2. Add tofu and simmer. 3. Garnish with green onion.",
    servings=2
)

mapo_tofu.display_recipe()
mapo_tofu.add_ingredient("tofu", 50)
mapo_tofu.display_recipe()
