...
def add_question():
  ...
  correct_index = int(input(f'Enter the number of the correct opion (1-4):\n')) - 1
...
def write_to_file(filename='quiz_questions.txt'):
    with open(filename, 'w', encoding='utf-8') as f:
        for idx, q in enumerate(quiz_questions, 1):
            ...
            f.write(f"Answer: {chr(65 + q['correct_answer'])}\n\n")
...
def main():
    while True:
        add_question()
        cont = input("Add another question? (yes/no):\n").strip().lower()
        if cont != 'yes':
            break
    print("\nHere are your quiz questions:\n")
    display_questions()
    write_to_file()
    print("Questions saved to 'quiz_questions.txt'.")
